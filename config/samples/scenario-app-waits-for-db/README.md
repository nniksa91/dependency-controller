...
| File | Resource |
|------|----------|
| `db-deployment.yaml` | `demo-db` — nginx with `initContainers` `sleep 45` + readiness (AvailableReplicas) |
...
```sh
# Apply (order does not matter; controller reconciles continuously)
